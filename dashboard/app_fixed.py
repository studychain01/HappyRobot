...
with tab2:
    ...
    if conversations_data:
        ...
        # Apply status filter using load_classification
        if status_filter != "All":
            if status_filter == "Booked":
                filtered_conversations = [c for c in filtered_conversations 
                                        if c.get('load_classification') == 'successful']
            elif status_filter == "Not Booked":
                filtered_conversations = [c for c in filtered_conversations 
                                        if c.get('load_classification') == 'unsuccessful']
            elif status_filter == "Unknown":
                filtered_conversations = [c for c in filtered_conversations 
                                        if not c.get('load_classification')]
        ...
        for conv in filtered_conversations:
            with st.expander("•"):
                ...
                with col2:
                    ...
                    # Load booking status using load_classification field
                    load_classification = conv.get('load_classification')
                    if load_classification:
                        if load_classification.lower() == 'successful':
                            st.success("✅ Load: BOOKED")
                        elif load_classification.lower() == 'unsuccessful':
